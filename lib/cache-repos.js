...
export const repoNames = ['MySnippets', 'blog', 'easy-interview', 'leetcode', 'ng_blog', 'cn-city-select', 'lifetools', 'ModeSetting'];
...
export function generateCacheReposPromise () {
  var cacheReposPromise = new Promise((resolve, reject) => {
    github.repos.getForUser({
      ...
      var redis = new Redis();
      var promise = redis.set('repo:count', result.length).then((flag) => {
        if ('OK' === flag) {
