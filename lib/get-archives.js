...
export function getAllArticles () {
  ...
  var cached, promiseSet = [], articles = [];
  var redis = new Redis();
    ...
    promiseSet.push((function (index) {
      ...
      if (index === 0) {
        promise = promise.then((flag) => {
          redis.end();
          cached = flag;
          return (cached && onProd()) ? fetchCachedArticle(fileName) : fetchArticle(fileName);
        });
      }
      ...
      return () => {
        if (index < l - 1) {
          promise = promise.then((article) => {
            ...
            return (cached && onProd()) ? fetchCachedArticle(fileNames[index + 1]) : fetchArticle(fileNames[index + 1]);
          });
        } else {
          promise = promise.then((article) => {
            articles.push(article);
